...
describe('PaletteApp browser tests', () => {
    ...
    it('boots palette with theme Extension and provides both APIs', async () => {
    ...
    it('switches themes via UI buttons and updates CSS variables', async () => {
        ...
        const tokensWin = site.manager.windows().find((w) => w.view === 'tokens');
        if (tokensWin) site.manager.focus(tokensWin.id);
    ...
    it('renders the live preview view with themed elements', async () => {
        ...
        const previewWin = site.manager.windows().find((w) => w.view === 'preview');
        if (previewWin) site.manager.focus(previewWin.id);
