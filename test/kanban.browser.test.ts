...
describe('KanbanApp browser tests', () => {
    ...
    it('boots, starts the Application, and provides the KanbanApi', async () => {
        ...
        expect(api?.cardsInColumn('todo').length).toBe(2);
        expect(api?.cardsInColumn('in_progress').length).toBe(1);

        site.dispose();
    });

    it('renders board and composer views into the DOM', async () => {
        const site = await mountPart({
            parts: [{ id: 'kanban', contribution: KanbanApp }],
        });
        ...
        // All 4 columns rendered
        const columns = document.querySelectorAll<HTMLElement>('.kanban-column');
        expect(columns.length).toBe(4);

        // 5 cards across columns
        const cards = document.querySelectorAll<HTMLElement>('.kanban-card-item');
    ...
    it('declares commands, keys, views statically on the class', () => {
